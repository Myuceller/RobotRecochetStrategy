      ...
      <div className="mt-4 rounded border border-slate-200 bg-slate-50 p-3">
        <div className="mb-2 flex items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">
              Pick a difficulty, then generate a solver-checked puzzle.
            </p>
          </div>
          {isGeneratingPuzzle ? (
            <button
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className={isSelected ? 'text-xs text-slate-200' : 'text-xs text-slate-500'}>
                    {preset.minDepth}-{preset.maxDepth}
                  </span>
                </div>
                <div
        ...
        <div className="mt-3 grid gap-2 sm:grid-cols-[1fr_auto]">
          <button
            ...
            disabled={isGeneratingPuzzle}
            className="rounded bg-slate-900 px-4 py-3 text-base font-semibold text-white hover:bg-slate-700 disabled:cursor-not-allowed disabled:bg-slate-400"
          >
            {isGeneratingPuzzle
              ? 'Generating...'
          ...
          <button
            ...
            className="rounded border border-slate-300 bg-white px-4 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-40"
          >
            Reset Sample
          </button>
        </div>
        ...
        {selectedDifficulty === 'hard' ? (
          <p className="mt-2 text-xs text-slate-500">
            Hard는 생성에 시간이 더 걸리거나 fallback puzzle로 대체될 수 있습니다.
          </p>
        ) : null}
      </div>
